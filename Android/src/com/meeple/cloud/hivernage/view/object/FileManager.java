...
public class FileManager {
			...
			while ((line = br.readLine()) != null) {
				...
				if (count != 0) {
					...
					if (campingsLine.length == 5) {
						...
						Camping ca = new Camping(campingsLine[0], campingsLine[1], campingsLine[2], accompte, campingsLine[4]);
			...
			while ((line = br.readLine()) != null) {
				...
				if (count != 0) {
					...
					if (clientLine.length > 7) {
						...
						if (client == null) {
							client = new Client(clientLine[0], clientLine[1], clientLine[2], clientLine[3], clientLine[4], clientLine[6], null);

							client.addHivernage(new Hivernage(Integer.parseInt(clientLine[5])));

							Services.clientService.create(client);
						}
					}
					if (clientLine.length == 14 && client != null) {
						//Immatriculation;Gabarit;Camping/HIVERNAGE;Emplacement/Hangar;Entrée/X;Sortie/Y;-/Angle;Observation
						...
						caravane = new Caravane(clientLine[7], clientLine[14], gabarit, null);
						...
						//
							...
							EmplacementCamping emplCamp = new EmplacementCamping(clientLine[10], caravane);
							try {
	...
	public static void writeClientsToCSV(Context context) {
		...
		File campings_file = new File(documents, CLIENTS_FILENAME);
